"""Generate scatter plots: PPG RMSSD vs ECG RMSSD for each step vs baseline.

For each step, produces a figure with 2 rows × 4 cols (baseline top, step bottom).
Uses best channel per device with bandpass preprocessing, ecg_interp=True (symmetric).

Must be run from the heuristic_baselines directory.
"""
import sys
import warnings
...
# Imports (this file lives inside heuristic_baselines/)
...
# -- Core functions (from full_step_matrix_v2.py) --
...
def compute_rmssd(peaks_int, peaks_float, fs, *, do_interp, do_ibi_correct, do_threshold):
    """Generic RMSSD from peak arrays (used for PPG side)."""
...
OUT_DIR = Path(__file__).resolve().parent / "outputs" / PARTICIPANT / "scatter_plots"
