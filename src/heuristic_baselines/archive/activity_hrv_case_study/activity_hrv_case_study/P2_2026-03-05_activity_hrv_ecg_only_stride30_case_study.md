# P2 ECG-only HRV 与活动日志对齐案例分析（2026-03-05）
...
- window-level sidecar CSV：`P2_2026-03-05_activity_hrv_ecg_only_stride30_window_aligned.csv`
- 可视化图：`P2_2026-03-05_activity_hrv_ecg_only_stride30_plot.png`

## 数据质量

- 当天 ECG-only 候选窗口数：463
- ECG-QC 合格窗口数：90 (19.4%)
- ECG-QC 不合格窗口数：373
- ECG-QC 合格窗口中心时间范围：2026-03-05 03:03:00 到 2026-03-05 03:53:30
- 当天解析出的 activity interval 数：1
- HRV 覆盖时间内的 point/unpaired activity event 数：0
- 未匹配到 activity 的窗口比例：100.0%

这个 ECG-only 版本用于回答全天 ECG-derived HRV 与 activity log 的关系；它不适合直接作为 PPG model training dataset，因为没有要求三设备 PPG 同时可用。

## 单日 HRV 波动范围

- RMSSD 中位数/IQR：64.86 ms / 6.40 ms
- RMSSD 最小值/最大值：55.01 ms / 72.56 ms
- 相邻窗口之间最大的 RMSSD 变化：8.69 ms
- 约 30 分钟内最大的 RMSSD 变化：11.77 ms
- SDNN 中位数/IQR：68.49 ms / 17.93 ms
- ECG HR 中位数/IQR：63.57 bpm / 1.75 bpm
...
_没有 HRV 覆盖范围内的 activity。_

## 未匹配 Activity 的窗口

| 开始中心时间 | 结束中心时间 | 窗口数 | 说明 |
| --- | --- | --- | --- |
| 2026-03-05 03:03:00 | 2026-03-05 03:53:30 | 90 | 该时间段没有可靠 activity interval 与 HRV window 重叠 |
...
![P2 ECG-only HRV activity plot](P2_2026-03-05_activity_hrv_ecg_only_stride30_plot.png)
