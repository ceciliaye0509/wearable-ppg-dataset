...
def _ibi_metrics(peaks: np.ndarray, signal: np.ndarray, fs: float, correction_threshold: float) -> dict[str, float]:
    peaks = np.asarray(peaks, dtype=np.int64)
    if peaks.size < 3:
        return {
            "n_peaks": float(peaks.size),
            "ppg_rmssd_ms": np.nan,
            "ppg_sdnn_ms": np.nan,
    ...
    # 完整抛物线峰值细化有价值，但在全队列规模上开销较高。
    # v1 先用整数采样点峰值冻结 detector/QC；峰值细化可以作为 v1.1 增量再评估。
    ...
    sqi = _fast_ppg_sqi(signal, fs, peaks)
    return {
        "n_peaks": float(peaks.size),
        "ppg_rmssd_ms": rmssd,
        "ppg_sdnn_ms": sdnn,
...
def _recompute_one_channel(raw: np.ndarray, fs: float, method: PeakMethod) -> dict[str, float | str]:
    ...
    if not np.isfinite(x).all():
        return {
            ...
            "polarity": "invalid",
            **_ibi_metrics(np.empty(0, dtype=np.int64), np.zeros(0), fs, method.correction_threshold),
        }
    if method.detector == "neurokit_raw":
        ...
        return {
            ...
            "detector": method.detector,
            "polarity": "neurokit",
            **_ibi_metrics(peaks, clean, fs, method.correction_threshold),
        }
    ...
    return {
        ...
        "detector": method.detector,
        "polarity": polarity,
        **_ibi_metrics(peaks, signal_for_metrics, fs, method.correction_threshold),
    }
...
def _write_readme(out_dir: Path, channel_df: pd.DataFrame, summary: pd.DataFrame, frozen: pd.DataFrame, frozen_eval: pd.DataFrame) -> None:
    lines = [
        "# 设备级 Baseline v1：重新计算 PPG 峰值",
        "",
        "本报告从 `ppg_resampled` 重新计算 PPG 峰值、IBI、SQI、QC 和 HRV，不沿用 v0 的 `Earring IR / Ring SQI-best / Watch SQI-best` 结论。",
        "",
        "## 重要原则",
        "",
        "- 不训练模型。",
        ...
        "- 通道选择、QC 覆盖率和最佳策略全部基于 v1 重新计算结果。",
    ...
    lines.extend([
        "",
        "## 冻结 v1 在 strict_reference / training 上的结果",
        "",
    ...
    lines.extend([
        ...
        "| 文件 | 含义 |",
        "|---|---|",
        "| `v1_recomputed_channel_metrics.csv` | 重新计算的通道级 PPG HRV / SQI / IBI 指标 |",
        "| `v1_strategy_summary.csv` | 每个候选策略的 MAE/RMSE/R/Bias/覆盖率 |",
        "| `v1_frozen_training_strategies.csv` | 在 training_stride30 上重新冻结的每设备策略 |",
        "| `v1_frozen_strategy_eval.csv` | 冻结策略在 training 和 strict_reference 上的结果 |",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="评估重新计算 PPG 峰值后的 v1 device-wise 启发式 baseline。")
    parser.add_argument(
        "--strict-dir",
    ...
    parser.add_argument(
        "--out-dir",
        default=str(config.HEURISTIC_RESULT_ROOT / "all_participants_devicewise_baseline_v1_recomputed_peaks"),
    )
    ...
    channel_csv = out_dir / "v1_recomputed_channel_metrics.csv"
    participants = tuple(p.strip() for p in args.participants.split(",") if p.strip())
    peak_methods = PEAK_METHODS + NEUROKIT_METHODS if args.include_neurokit else PEAK_METHODS
    ...
    summary.to_csv(out_dir / "v1_strategy_summary.csv", index=False)
    frozen.to_csv(out_dir / "v1_frozen_training_strategies.csv", index=False)
    frozen_eval.to_csv(out_dir / "v1_frozen_strategy_eval.csv", index=False)
    run_summary = {
