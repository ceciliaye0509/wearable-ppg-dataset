# encoding=utf-8
"""
hrv_ext_group4_kazemi_decoder_score.py
========================================
...
def setup_dataloaders_peak_group(
    args,
    train_pids,
    val_pids,
    test_pids,
):
    """Build leakage-safe participant-grouped loaders for the peak task."""
    if args.task != "peak":
        raise ValueError("Grouped runner currently supports --task peak only")
    ...
    if train_set & val_set or train_set & test_set or val_set & test_set:
        raise ValueError(
            "Participant leakage: train/validation/test sets must be disjoint"
        )

    available = set(discover_hrv_participants(args.data_dir))
    requested = train_set | val_set | test_set
    missing = sorted(requested - available)
    if missing:
        raise ValueError(f"Missing participants in data directory: {missing}")

    train_rows = [_load_one(args.data_dir, pid, args) for pid in train_pids]
    val_rows = [_load_one(args.data_dir, pid, args) for pid in val_pids]
    test_rows = {
        pid: _load_one(args.data_dir, pid, args)
        for pid in test_pids
    }

    fs_values = {
        float(row["tgt_fs"])
        for row in train_rows + val_rows + list(test_rows.values())
    }
    if len(fs_values) != 1:
        raise ValueError(f"Inconsistent target sampling rates: {fs_values}")
    args._tgt_fs = fs_values.pop()

    def make_dataset(rows):
        x = np.concatenate([row["X"] for row in rows], axis=0)
        peaks = np.concatenate([row["peak"] for row in rows], axis=0)
        return TensorDataset(torch.tensor(x), torch.tensor(peaks))

    batch_size = args.batch_size
    train_loader = DataLoader(
        _TripletDS(make_dataset(train_rows)),
        batch_size=batch_size,
        shuffle=True,
    )
    val_loader = DataLoader(
        _TripletDS(make_dataset(val_rows)),
        batch_size=batch_size,
        shuffle=False,
    )
    ...
    test_loaders = {}
    test_metadata = {}
    for pid, row in test_rows.items():
        dataset = TensorDataset(
            torch.tensor(row["X"]),
            torch.tensor(row["peak"]),
        )
        test_loaders[pid] = DataLoader(
            _TripletDS(dataset),
            batch_size=batch_size,
            shuffle=False,
        )
        test_metadata[pid] = {
            "y_ms": row["y_ms"],
            "base": row["base"],
            "n_windows": int(len(row["X"])),
        }

    return [train_loader], val_loader, test_loaders, test_metadata
...
class PeakLoss(nn.Module):
    ...
    def __init__(
        ...
        hard_negative_exclusion_ms=150.0,
        fs=50.0,
    ):
        super().__init__()
        ...
        self.hard_negative_exclusion_ms = float(hard_negative_exclusion_ms)
        self.fs = float(fs)
    ...
    def forward(self, logits, target):
        ...
        loss = bce + self.dice_weight * (1.0 - dice)

        if self.hard_negative_weight > 0.0:
            radius = max(
                1,
                int(round(self.hard_negative_exclusion_ms / 1000.0 * self.fs)),
            )
            peak_core = (target >= 0.50).to(logits.dtype).unsqueeze(1)
            ...
            negative_logits = logits.masked_fill(protected, float("-inf"))
            k = max(1, int(round(logits.shape[1] * self.hard_negative_fraction)))
            k = min(k, logits.shape[1])
            hardest = torch.topk(negative_logits, k=k, dim=1).values
            finite = torch.isfinite(hardest)
            if finite.any():
                loss = loss + self.hard_negative_weight * F.softplus(
                    hardest[finite]
                ).mean()
    ...
    if args.task == "peak":
        return PeakLoss(
            ...
            hard_negative_weight=float(
                os.environ.get("PEAK_HARD_NEG_WEIGHT", "0.0")
            ),
            hard_negative_fraction=float(
                os.environ.get("PEAK_HARD_NEG_FRACTION", "0.01")
            ),
            hard_negative_exclusion_ms=float(
                os.environ.get("PEAK_HARD_NEG_EXCLUSION_MS", "150.0")
            ),
            fs=float(getattr(args, "_tgt_fs", args.resample_hz)),
        )
    return nn.MSELoss()
...
def test_peak(
    ...
    # Expose exact held-out predictions to the grouped runner so it can write
    # one pooled CSV without re-running model inference.
    args._last_peak_eval = {
        "participant": current_participant,
        "true_hrv": np.asarray(true_hrv, dtype=float),
        "predicted_hrv": np.asarray(predicted_hrv, dtype=float),
        "valid": np.asarray(valid, dtype=bool),
        "tp": int(total_tp),
        "fp": int(total_fp),
        "fn": int(total_fn),
        "true_peak_counts": np.asarray(true_peak_counts, dtype=float),
        "predicted_peak_counts": np.asarray(predicted_peak_counts, dtype=float),
        "correction_ratios": np.asarray(correction_ratios, dtype=float),
    }
...
def select_peak_threshold(
    ...
    """
    Select the peak probability threshold on validation participants using
    event F1 penalized by peak-count error and RR-correction burden.
    Test participants are never used for threshold selection.
    """
    ...
    best_details = None
    count_weight = float(os.environ.get("PEAK_COUNT_PENALTY", "0.5"))
    correction_weight = float(
        os.environ.get("PEAK_CORRECTION_PENALTY", "0.25")
    )
    ...
    for threshold in thresholds:
        ...
        for prob, target in zip(probabilities, targets):
            ...
            _, _, _, correction_ratio = _hrv_from_prob(
                prob,
                fs,
                thr=threshold,
                ibi_method=args.ibi_method,
            ...
            if np.isfinite(correction_ratio):
        ...
        count_ratio = total_predicted / max(total_true, 1)
        mean_correction = (
            float(np.mean(correction_ratios)) if correction_ratios else 1.0
        )
        score = (
            f1
            - count_weight * abs(count_ratio - 1.0)
            - correction_weight * mean_correction
        )

        if score > best_score:
            best_score = score
            best_threshold = float(threshold)
            best_details = (f1, count_ratio, mean_correction)

    best_f1, best_count_ratio, best_correction = best_details

    if not quiet:
        print(
            f"    Selected peak threshold: {best_threshold:.2f} "
            f"| validation F1: {best_f1:.3f} "
            f"| decoder score: {best_score:.3f} "
            f"| count ratio: {best_count_ratio:.3f} "
            f"| correction ratio: {best_correction:.3f}"
        )
