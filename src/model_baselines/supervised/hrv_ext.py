...
def _hrv_from_prob(prob, fs, thr=0.5, min_rr_ms=300):
    # simple peak picking: local maxima + threshold + min distance
    idx = []
    d = int(min_rr_ms / 1000 * fs)
    i = 1
    while i < len(prob) - 1:
        if prob[i] >= thr and prob[i] >= prob[i-1] and prob[i] >= prob[i+1]:
            if not idx or i - idx[-1] >= d:
                idx.append(i)
        i += 1
    if len(idx) < 3:
        return np.array([np.nan, np.nan])
    t = np.array(idx) / fs * 1000.0
    rr = np.diff(t)
