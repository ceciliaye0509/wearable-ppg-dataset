...
import hrv_ext_group4_kazemi_decoder_score as H
...
def train(args, train_loaders, val_loader, model, device, optimizer, criterion,
    ...
    for epoch in range(args.n_epoch):
        ...
        val_decoder_score = None
        if args.task == 'peak':
            _, val_decoder_score = H.select_peak_threshold(
                val_loader, model, device, args,
                return_score=True, quiet=True,
            )

        improved = (
            val_decoder_score > best_val_score
            if args.task == 'peak'
        ...
        if improved:
            ...
            if val_decoder_score is not None:
                best_val_score = val_decoder_score
            best_state    = deepcopy(model.state_dict())
            epochs_no_imp = 0
            torch.save({'model_state_dict': model.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict()},
                       checkpoint_path)
            metric_text = (
                f" | Val decoder score: {val_decoder_score:.3f}"
                if val_decoder_score is not None else ""
            )
