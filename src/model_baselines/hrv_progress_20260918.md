# PPG→HRV Weekly Progress Update
...
3. A new **validation decoder score** nearly eliminates P1 peak-count bias and reduces SDNN MAE from `31.01` to `13.43 ms`.
...
## 9. In progress this week

### Kazemi + validation decoder score

A new grouped 4-fold run is being submitted with checkpoint and threshold selected using validation participants only:
...
Primary question: can validation decoder-score selection repair fold 0/1 peak-count bias without degrading the strong fold 2/3 results?

## 10. Next steps

1. Finish Kazemi + decoder-score grouped 4-fold and compare participant-level paired changes.
2. Add strict and delay-aligned event metrics to all peak-model reports.
3. Develop an IBI-sensitive validation objective for RMSSD.
4. Match qppg, Own PeakNet, Kazemi, PulsePPG, and PaPaGei on identical held-out windows and report native/common coverage.
5. Run the proposed hybrid baseline:

   `qppg HRV + quality features + frozen embedding → nested Ridge residual correction`

6. Extend the frozen-embedding Ridge alpha grid beyond `1000` and check whether selection remains boundary-limited.

## 11. Meeting take-away

> This week established that our peak networks are learning cardiac morphology, but their main failure is cross-participant event decoding and calibration. The finalized Own PeakNet over-detects systematically. Kazemi captures substantially more HRV variation and performs very well in two grouped folds, but its absolute calibration is unstable. Validation decoder-score selection successfully corrects P1 over-detection and is now being evaluated across all participant groups. Frozen PulsePPG + nested Ridge remains the strongest stable learned baseline, while the longer-term direction is a leakage-safe hybrid combining heuristic HRV with learned residual correction.
